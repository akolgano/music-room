...
class ApiService {
  ...
  Future<PendingRequestsResponse> getPendingFriendRequests(String token) => 
      _get('/users/pending_friend_requests/', PendingRequestsResponse.fromJson, token: token);
  ...
  Future<void> addTrackFromDeezer(int trackId, String token) => _postVoid('/tracks/add_from_deezer/$trackId/', {}, token: token);
  ...
  Future<void> updatePlaylist(String playlistId, String token, UpdatePlaylistRequest request) => 
      _patch('/playlists/playlists/$playlistId/', request, (data) => data, token: token);
  ...
  Future<Map<String, VoteStats>> getTrackVotes(String playlistId, String token) async {
    ...
    return (response['track_votes'] as Map<String, dynamic>).map(
  ...
  Future<DevicesResponse> getAllUserDevices(String token) => 
      _get('/devices/all/', DevicesResponse.fromJson, token: token);
  ...
  Future<PermissionResponse> checkControlPermission(String deviceUuid, String token) => 
      _get('/devices/$deviceUuid/can-control/', PermissionResponse.fromJson, token: token);
  ...
  Future<Map<String, dynamic>> getProfilePrivateData(String? token) => 
      _get('/profile/private/', (data) => data, token: token);

  Future<Map<String, dynamic>> getProfileFriendData(String? token) => 
      _get('/profile/friend/', (data) => data, token: token);
  ...
  Future<void> updatePublicBioData(String? token, String? bio) => updateProfile(token!, {'bio': bio});

  Future<void> updatePublicBasic(String token, PublicBasicUpdateRequest request) => 
      updateProfile(token, request.toJson());
  ...
  Future<void> updateMusicPreferencesData(String? token, List<String>? musicPreferences) => 
      updateProfile(token!, { if (musicPreferences != null) 'music_preferences': musicPreferences });

  Future<void> googleLinkData(String type, String? token, String idToken) => 
      googleLink(token!, SocialLinkRequest(type: type, idToken: idToken));
}
