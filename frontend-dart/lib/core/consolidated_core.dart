...
class AppValidators {
  static String? required(String? value, [String? fieldName]) =>
      (value?.trim().isEmpty ?? true) ? 'Please enter ${fieldName ?? 'this field'}' : null;

  static String? email(String? value) {
    if (value?.isEmpty ?? true) return 'Please enter an email';
    final emailRegex = RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$');
    return !emailRegex.hasMatch(value!.trim()) ? 'Please enter a valid email address' : null;
  }
  ...
  static String? username(String? value) {
    ...
    return !usernameRegex.hasMatch(value) ? 'Username can only contain letters, numbers, and underscores' : null;
  }
  ...
  static String? phoneNumber(String? value, [bool required = false]) {
    ...
    return (digitsOnly.length < 7 || digitsOnly.length > 15) ? 'Phone number must be between 7 and 15 digits' : null;
  }

  static String? Function(String?) maxLength(int maxLength, [String? fieldName]) =>
      (String? value) => (value != null && value.length > maxLength) 
          ? '${fieldName ?? 'Field'} must be less than $maxLength characters' : null;

  static String? playlistName(String? value) {
    if (value?.isEmpty ?? true) return 'Please enter a playlist name';
    return value!.length > 100 ? 'Playlist name must be less than 100 characters' : null;
  }
...
mixin StateManagement on ChangeNotifier {
  ...
  Future<T?> executeAsync<T>(Future<T> Function() operation, {String? successMessage, String? errorMessage, bool showLoading = true}) async {
...
class AppTheme {
  ...
  static Widget buildFormCard({required String title, IconData? titleIcon, required Widget child}) {
    return Card(
      color: surface,
      elevation: 4,
